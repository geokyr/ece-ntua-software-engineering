...
my_parser = argparse.ArgumentParser(description='returns all the passes between two toll \
    providers in a specific period with ids and period provided in the params of the url')
...
url = 'http://127.0.0.1:9103/PassesAnalysis/' + args.op1 + '/' + args.op2 + '/' + args.datefrom + '/' + args.dateto + '?format=' + args.format
r = requests.get(url)
